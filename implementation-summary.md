...
**位置：** `docs/implementation/03-schema-system.md`
...
**位置：** `docs/implementation/04-state-management.md`
...
**位置：** `docs/implementation/05-instruction-generation.md`
...
```
用户: openspec new change add-auth
  ↓
CLI 解析和验证
  ↓
...
```
用户: openspec instructions proposal --change add-auth
  ↓
加载上下文:
  ├─ 解析 Schema
  ├─ 构建依赖图
...
```
用户: openspec status --change add-auth
  ↓
加载上下文
  ↓
检测完成状态:
...
1. 阅读 [架构总览](./implementation/01-architecture.md)
2. 理解 [Artifact Graph](./implementation/02-artifact-graph.md)
3. 学习 [Schema 系统](./implementation/03-schema-system.md)

### 进阶

1. 深入 [状态管理](./implementation/04-state-management.md)
2. 掌握 [指令生成](./implementation/05-instruction-generation.md)
3. 研究源码实现
...
**完整文档索引：** [docs/implementation/README.md](./implementation/README.md)
